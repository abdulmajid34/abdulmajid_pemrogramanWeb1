...
async function loadData() {
    ...
    try {
        const res  = await fetch(`${API}?action=list_dosen`);
...
function renderTable(data) {
    ...
    setTableInfo(`Menampilkan <strong>${data.length}</strong> data dosen`);
...
function siapkanTambah() {
    ...
    document.getElementById('modalDosenLabel').textContent = 'Tambah Dosen';
    document.getElementById('btnSimpan').innerHTML = '<i class="bi bi-plus-circle me-1"></i>Tambah';
}

async function siapkanEdit(id) {
    try {
        const res  = await fetch(`${API}?action=get_single_dosen&id=${id}`);
        const json = await res.json();
        if (!json.success) throw new Error(json.message);

        const d = json.data;
        document.getElementById('dosenId').value      = d.id;
        document.getElementById('inputNamaDosen').value = d.nama;
        document.getElementById('inputAlamat').value  = d.alamat;

        document.getElementById('modalDosenLabel').textContent = 'Edit Dosen';
...
async function simpanData(event) {
    ...
    const fd  = new FormData(document.getElementById('formDosen'));

    btn.disabled = true;
    btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Menyimpan...';

    try {
        const res  = await fetch(`${API}?action=save_dosen`, { method: 'POST', body: fd });
        const json = await res.json();
        if (!json.success) throw new Error(json.message);
...
async function hapusData(id) {
    ...
    try {
        const res  = await fetch(`${API}?action=delete_dosen`, { method: 'POST', body: fd });
        const json = await res.json();
        if (!json.success) throw new Error(json.message);
